...
use crate::{
    ...
    shared::error::{AuthError, DatabaseSnafu, Error, GenericSnafu, InvalidCredentialsSnafu},
};
...
impl AuthSession<AuthBackendSqlite> {
    ...
    pub async fn logout(&mut self) -> Result<(), Error> {
        ...
        inner
            ...
            .flush()
            .await
            .map_err(|_| AuthError::SessionFlushFailed)?;
    ...
    pub async fn update_session(&mut self) -> Result<(), Error> {
        ...
        inner
            ...
            .await
            .map_err(|_e| AuthError::SessionUpdateFailed)?;
...
impl AuthBackend<SqliteStore> for AuthBackendSqlite {
    ...
    async fn authenticate(&self, creds: &Credentials) -> Result<Option<User>, Self::Error> {
        ...
        .map_err(|_| {
            GenericSnafu {
                reason: "Failed to join password verification task",
            }
            .build()
        })?
    }
...
pub fn verify_password(password: impl AsRef<[u8]>, hash: &str) -> Result<(), Error> {
    let parsed_hash = PasswordHash::new(hash).map_err(|_| AuthError::PasswordHashFailed)?;
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        // In integration tests, bypass CSRF verification to simplify testing
        #[cfg(any(test, feature = "integration-tests"))]
        {
            return Ok(VerifiedCSRFToken);
        }

        #[cfg(not(any(test, feature = "integration-tests")))]
        {
            let csrf_token = parts.extensions.get::<CsrfToken>().ok_or((
                http::StatusCode::INTERNAL_SERVER_ERROR,
                "Can't extract CsrfToken from extensions",
            ))?;

            let token_header = parts
                .headers
                .get("X-CSRF-Token")
                .ok_or((http::StatusCode::BAD_REQUEST, "Missing CSRF token header"))?
                .to_str()
                .map_err(|_| (http::StatusCode::BAD_REQUEST, "Invalid CSRF token header"))?;

            csrf_token.verify(token_header).map_err(|_| {
                (
