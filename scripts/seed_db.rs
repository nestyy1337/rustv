...
async fn main() -> Result<(), sqlx::Error> {
    ...
    for (imdb_id, title, production_company, year, genre, available) in movies {
        sqlx::query(
            "INSERT OR IGNORE INTO movies (imdb_id, title, production_company, release_year, genre, available)
             VALUES (?, ?, ?, ?, ?, ?)",
        )
        .bind(imdb_id)
        .bind(title)
